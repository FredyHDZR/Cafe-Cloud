...
class OrderCompletedEnvelope(BaseModel):
    model_config = ConfigDict(extra="ignore", frozen=True)
...
def parse_order_completed(fields: Mapping[str, str]) -> OrderCompletedEnvelope:
    raw = _decode(fields)
    _require_event_type(raw)
    _require_supported_version(raw)
    try:
        return OrderCompletedEnvelope.model_validate(raw)
...
def _require_event_type(raw: Mapping[str, Any]) -> None:
    ...
    if event_type != ORDER_COMPLETED_TYPE:
...
def _require_supported_version(raw: Mapping[str, Any]) -> None:
    ...
    if version not in SUPPORTED_VERSIONS:
        message = f"event_version {version} no soportada para {ORDER_COMPLETED_TYPE}"
