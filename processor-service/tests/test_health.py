...
def test_liveness_does_not_look_at_any_dependency(client: TestClient) -> None:
    ...
    assert response.json() == {"status": "alive", "service": "processor-service"}


def test_health_reports_every_dependency_when_all_are_up(
    client: TestClient, health: list[Dependency]
) -> None:
    health.extend([Dependency(name="postgres", probe=up), Dependency(name="redis", probe=up)])
...
def test_redis_down_takes_the_consumer_out_of_service(
    client: TestClient, health: list[Dependency]
) -> None:
    # Aqui Redis si es critica: sin stream no hay nada que consumir.
    health.extend([Dependency(name="postgres", probe=up), Dependency(name="redis", probe=down)])
