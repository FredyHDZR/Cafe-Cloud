...
PURGE_POLICY = PurgePolicy(outbox_retention_hours=24, batch_size=1000)
...
def test_publishes_every_pending_row_and_marks_it_published() -> None:
    ...
    assert [entry[0] for entry in stream.published] == ["orders.completed", "orders.completed"]
...
def test_the_purge_commits_and_reports_what_it_deleted() -> None:
    outbox = FakeOutboxStore([])
    unit_of_work = FakeUnitOfWork()
    service = PurgeOutboxService(outbox=outbox, unit_of_work=unit_of_work, policy=PURGE_POLICY)
