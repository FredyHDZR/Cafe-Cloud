...
from app.infra.metrics import CONTENT_TYPE, METRICS_COLLECTION_ERRORS, ORDERS_CREATED
...
def test_metrics_are_served_in_the_prometheus_exposition_format(
    ...
    assert "# TYPE cafecloud_orders_created_total counter" in response.text
...
def test_the_service_is_named_in_its_own_metrics(
    ...
    assert 'cafecloud_service_info{service="orders-service",version="0.1.0"} 1.0' in body


def test_creating_an_order_moves_the_counter(client: TestClient, collector: FakeCollector) -> None:
    before = sample("cafecloud_orders_created_total")

    ORDERS_CREATED.inc()

    assert sample("cafecloud_orders_created_total") == before + 1
    assert f"cafecloud_orders_created_total {before + 1}" in client.get("/metrics").text
