...
class PurgeOutboxService:
    ...
    async def run(self) -> PurgeOutcome:
        ...
        idempotency_keys = await self._idempotency.delete_expired(
            grace_hours=self._policy.idempotency_grace_hours, limit=self._policy.batch_size
        )
        await self._unit_of_work.commit()
        outcome = PurgeOutcome(outbox_rows=outbox_rows, idempotency_keys=idempotency_keys)
        _log_purge(outcome)
        return outcome
...
def _log_purge(outcome: PurgeOutcome) -> None:
    context = log_context(
        outbox_rows=outcome.outbox_rows, idempotency_keys=outcome.idempotency_keys
    )
    if outcome.outbox_rows or outcome.idempotency_keys:
